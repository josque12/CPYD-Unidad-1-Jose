...
int main() {
    ...
    kant.LeftFork() = fork4;
    kant.RightFork() = fork5;

    Philosopher aristoteles("Aristoteles");
    aristoteles.LeftFork() = fork5;
    aristoteles.RightFork() = fork1;
    ...
    std::thread threadAristoteles([&](){ aristoteles.eat(); });

    threadSocrates.join();
    threadDiogenes.join();
    threadPlaton.join();
    threadKant.join();
    threadAristoteles.join();
